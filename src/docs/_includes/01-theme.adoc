...
TIP: See {link-admin-openshift}/pages/form/forms.xhtml[showcase forms page^] to get an idea.
...
│   |
    ...
    └─ variables.less
----
<1> Bootstrap variables and https://css-tricks.com/snippets/css/useful-css3-less-mixins/[mixins^]
<2> Built in skins
<3> Primefaces components
<4> Components and admin-lte less files are included in theme.less

After compilation in will generate a single theme.css with admin-lte, bootstrap.
...
To get your hands dirty with admin theme it is recommended to use <<Admin Designer>> in combination with http://brackets.io/[Brackets].
...
NOTE: theme.less is already brackets aware so you just need to change any component less file and see the results.
