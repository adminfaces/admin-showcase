...
{link-adminfaces}[AdminFaces] is an open source project which brings {link-bootstrap}[Bootstrap^] and {link-adminlte}[AdminLTE^] to your application via a {link-primefaces}[PrimeFaces^] theme and {link-jsf}[JSF^] `responsive` template.
